...
### Added
- **Auto-reconnect: per-connection RetryConfig with exponential backoff** — new "Automatic Reconnection" section in the connection dialog Advanced tab allows configuring retry behavior per connection: enable/disable auto-reconnect, maximum attempts (1–10), initial delay (100–30000ms), and maximum delay (1000–120000ms); uses exponential backoff with 2× multiplier; when a connection drops unexpectedly, the retry config determines poll interval and maximum polling duration instead of hardcoded 5s/300s values; `RetryConfig` is serialized with `#[serde(default)]` for full backward compatibility — existing connections work unchanged
- **Import: multi-file batch import** — new "Multiple Files (batch)" source in the Import dialog allows selecting multiple files at once (CSV, RDP, VV, RCN, JSON, RTSZ, MobaXterm, XML, YAML); files are imported sequentially with per-file progress reporting; when the total connection count exceeds the batch threshold (>10), `BatchImporter` from rustconn-core processes them in configurable batch sizes with cancellation support; format is auto-detected from file extension 

### Improved
- **Import: success messages now use i18n_f()** — the import success toast previously used `format!()` with hardcoded English text; now uses `i18n_f()` with `{}` placeholders for proper localization of "Imported N connections to 'X' group" messages
...
- **Code quality: `performance/mod.rs` decomposed into submodules** — the monolithic 2210-line file is now split into 10 focused modules: `metrics.rs` (PerformanceMetrics, TimingGuard), `debouncer.rs`, `lazy.rs`, `memory.rs` (MemoryTracker, MemoryOptimizer, MemoryPressure, MemoryBreakdown), `batch.rs` (BatchProcessor), `scroller.rs` (VirtualScroller), `interner.rs` (StringInterner), `compact_string.rs`, `pool.rs` (ObjectPool), `shrinkable_vec.rs`; public API unchanged; tests moved to respective modules
- **Code quality: `cli_download.rs` decomposed into submodules** — the monolithic 3391-line file is now split into 10 focused modules: `components.rs`, `detection.rs`, `download.rs`, `extract.rs`, `install.rs`, `install_pip.rs`, `install_custom.rs`, `install_cloud.rs`, `uninstall.rs`, `update.rs`; public API unchanged
- **Code quality: MainWindow credential resolution extracted** — 9 credential-related methods (~920 lines) moved from `window/mod.rs` to `window/credentials.rs`: `start_connection_with_credential_resolution`, `handle_resolved_credentials`, `handle_rdp_credentials`, `handle_rdp_credentials_internal`, `handle_vnc_credentials`, `handle_vnc_credentials_internal`, `start_rdp_with_password_dialog`, `start_rdp_session_with_credentials`, `start_vnc_with_password_dialog`
- **Code quality: MainWindow session lifecycle extracted** — 5 session lifecycle methods (~760 lines) moved from `window/mod.rs` to `window/session_lifecycle.rs`: `setup_session_logging`, `setup_activity_monitoring`, `deliver_activity_notification`, `setup_child_exited_handler`, `setup_logging_handlers`; `window/mod.rs` reduced from 5395 to ~3700 lines (−31%)

### Fixed
- **CSV import: removed potential panic on empty group path** — `resolve_group_path()` previously used `expect()` which would panic if a CSV row contained an empty or whitespace-only group path; now returns `ImportError::InvalidEntry` and the connection is added to the skipped list instead of crashing the import
- **VNC/RDP/SPICE embedded: connection hangs in "Connecting" when remote port unreachable through SSH tunnel** — when using a jump host, `wait_for_tunnel_ready()` only verified that the local tunnel port was listening, but did not check whether the remote service was actually reachable through the tunnel; if the remote VNC/RDP/SPICE port was down, the embedded client would receive `Connection refused` / `early eof` from SSH and hang indefinitely in "Connecting" state; now `probe_tunnel_remote()` is called after tunnel readiness to verify end-to-end connectivity — if the remote port is unreachable, the connection fails immediately with a clear error instead of hanging
